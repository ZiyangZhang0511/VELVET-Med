...
def process_row(row, nii_dir):

    images_dir, depth, height, width = row
    spatial_size = (depth, height, width)

    generate_nii(images_dir, spatial_size, nii_dir)
...
def main():
    ...
    if args.target_case_id:
        info_data = info_data[info_data["case_id"].isin(args.target_case_id)]
    print(len(info_data))
    # return
    ...
    rows = list(zip(images_dir_list, depth_list, height_list, width_list))

    #### set up multiprocessing ###
    pool = multiprocessing.Pool(processes=None)
    worker = partial(process_row, nii_dir=args.nii_dir)
    for _ in tqdm(pool.imap_unordered(worker, rows), total=len(rows)):
        pass
